...
var callFmacStatus = rpc.declare({
...
return view.extend({
	load: function() {
		return L.resolveDefault(callFmacStatus(), '');
	},
	...
	render: function(status) {
		...
		poll.add(function() {
			return L.resolveDefault(callFmacStatus(), '').then(function(result) {
				dom.content(statusNode, result || _('Driver not loaded'));
			});
		}, 3);

		return E('div', {}, [
			E('h2', {}, _('FMAC Driver Status')),
			statusNode
		]);
	},
