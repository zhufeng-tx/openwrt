...
static inline void put_unaligned_le32(unsigned short val, unsigned char *p)
{
...
static inline unsigned short get_unaligned_le32(const unsigned char *p)
{
...
int hgic_iwpriv_hci_tx_cmd(char *ifname, uint16 opcode, uint8 *data, uint32 len)
{
    int ret = -1;
    uint32 cmd_len = len + 3;
    uint8 *cmd = malloc(cmd_len);
    if (cmd == NULL) return -ENOMEM;

    cmd[0] = (uint8)(opcode & 0xFF);
    cmd[1] = (uint8)((opcode >> 8) & 0xFF);
    cmd[2] = len;
    if (data != NULL) {
...
int hgic_iwpriv_blenc_start(char *ifname, int mode, int chan)
{
    uint8 data[2];
    data[0] = mode;
    data[1] = chan;
    return hgic_iwpriv_hci_tx_cmd(ifname, HCI_OPCODE(0x3f, 0x01), data, 2);
}
...
int hgic_iwpriv_blenc_set_coexist(char *ifname, int coexist, int dec_duty)
{
    ...
    data[0] = coexist;
    data[1] = dec_duty;
    return hgic_iwpriv_hci_tx_cmd(ifname, HCI_OPCODE(0x3f, 0x02), data, 2);
}

int hgic_iwpriv_send_blenc_data(char *ifname, char *data, int len)
{
    return hgic_iwpriv_blenc_send_hcidata(ifname, 0x80, data, len);
}
...
int hgic_iwpriv_blenc_set_advdata(char *ifname, char *adv_data, int len)
{
    ...
    return hgic_iwpriv_hci_tx_cmd(ifname, HCI_OPCODE(0x08, 0x08), adv_data, len);
}
...
int hgic_iwpriv_blenc_set_scanresp(char *ifname, char *scan_resp, int len)
{
    ...
    return hgic_iwpriv_hci_tx_cmd(ifname, HCI_OPCODE(0x08, 0x09), scan_resp, len);
}

int hgic_iwpriv_blenc_set_devaddr(char *ifname, char *addr)
{
    return hgic_iwpriv_hci_tx_cmd(ifname, HCI_OPCODE(0x08, 0x05), addr, 6);
}

int hgic_iwpriv_blenc_set_adv_interval(char *ifname, int interval)
{
    uint8 data[4];
    put_unaligned_le32(interval, data);
    return hgic_iwpriv_hci_tx_cmd(ifname, HCI_OPCODE(0x3f, 0x04), data, 4);
}

int hgic_iwpriv_blenc_set_adv_filter(char *ifname, int filter)
{
    return hgic_iwpriv_hci_tx_cmd(ifname, HCI_OPCODE(0x3f, 0x05), (uint8 *)&filter, 1);
}

int hgic_iwpriv_blenc_set_ble_llpkt_len(char *ifname, int length)
{
    uint8 data[2];
    put_unaligned_le16(length, data);
    return hgic_iwpriv_hci_tx_cmd(ifname, HCI_OPCODE(0x3f, 0x03), data, 2);
}

int hgic_iwpriv_blenc_start_adv(char *ifname, int start)
{
    return hgic_iwpriv_hci_tx_cmd(ifname, HCI_OPCODE(0x08, 0x0a), (uint8 *)&start, 1);
}
...
int hgic_iwpriv_set_power_level(char *ifname, int mode)
{
    return hgic_iwpriv_set_int(ifname, "power_level", mode);
}
